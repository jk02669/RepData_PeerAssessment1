...
author: "Joshua Klingel"
date: "Tuesday, April 14, 2015"
output: html_document
---

This is an R Markdown document for the Peer Assessment 1 assignment on the Reproducible Research course from Johns Hopkins.
...
library(data.table)
library(ggplot2) 
setwd("C:\\Users\\577512\\Documents\\R")
activity <- read.csv(file="activity.csv", header=T,sep=",", colClasses=c("numeric", "character", "numeric"))
```

Tidy the data and dates:
---
...
---
Now here we ignore the missing values(as directed).
...
head(steps_per_day)
```
We must make a histogram of the total number of steps taken per day, plotted with the appropriate interval.
...
Next, calculate the mean and median of number of steps taken per day.
```{r}
...
Next, find the 5-minute interval containing the maximum number of steps.
...
We create a function na_fill(data, pervalue) which the data arguement is the activity data frame and pervalue arguement is the steps_per_interval data frame.
```{r}
...
```
A zero output means there are zero missing values
